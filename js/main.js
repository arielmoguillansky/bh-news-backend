...
function deleteUser(){
  var deleteAction = `index.php?delete_user_id=${dataId}`
  window.location.href = deleteAction; 
}
function deleteArticle(){
  var deleteAction = `index.php?delete_article_id=${dataId}`
  window.location.href = deleteAction; 
}
function showModal(id, name, isUser){
  dataId = id;
  if(isUser) {
    document.querySelector('.modal').style.display = "flex";
    document.querySelector('.usersTable').style.filter = "blur(3px)";
    document.querySelector('.modal strong').innerHTML = name;
  } else {
    document.querySelector('.articleModal').style.display = "flex";
    document.querySelector('.articlesTable').style.filter = "blur(3px)";
    document.querySelector('.articleModal strong').innerHTML = name;
  }
}
function closeArticleModal(){
    document.querySelector('.articleModal').style.display = "none";
    document.querySelector('.articlesTable').style.filter = "none";
}
function closeModal(){
